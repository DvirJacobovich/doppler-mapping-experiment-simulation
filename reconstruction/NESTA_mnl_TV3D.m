function [xk,niter,residuals,outputData,opts] =NESTA_mnl_TV3D(A,At,b,muf,delta,m,n,l,opts)
...
for nl=1:MaxIntIter
    ...
    [xk,niter_int,res,out,optsOut] = Core_Nesterov_mnl_TV3D(...
        A,At,b,mu,delta,m,n,l,opts);
...
%---- internal routine for setting mu0 in the tv minimization case
    function th=ValMUTv(x,m,n,l)
        
        N = length(x);%n = floor(sqrt(N));
        
        % Vertical:
        M_neg=[-ones(m-1,n); zeros(1,n)];
        M_pos=[zeros(1,n); ones(m-1,n)];
        Dv = spdiags([reshape(repmat(M_neg,1,1,l),N,1) reshape(repmat(M_pos,1,1,l),N,1)], [0 1], N, N);
        
        % Horizontal:
        M_neg=[-ones(m,n-1) zeros(m,1)];
        M_pos=[zeros(m,1) ones(m,n-1)];
        Dh = spdiags([reshape(repmat(M_neg,1,1,l),N,1) reshape(repmat(M_pos,1,1,l),N,1)], [0 m], N, N);
        
        % Longitudinal:
        M_neg=[-ones(m,l-1) zeros(m,1)];
        M_pos=[zeros(m,1) ones(m,l-1)];
        
        M_neg_cube = repmat(M_neg,1,1,n);
        M_pos_cube = repmat(M_pos,1,1,n);
        
        M_neg_cube = permute(M_neg_cube,[1 3 2]);
        M_pos_cube = permute(M_pos_cube,[1 3 2]);
        
        Dl = spdiags([reshape(M_neg_cube,N,1) reshape(M_pos_cube,N,1)], [0 n*m], N, N);
        ...
        sk = sqrt(abs(Dhx).^2 + abs(Dvx).^2 + abs(Dlx).^2);
        th = max(sk);

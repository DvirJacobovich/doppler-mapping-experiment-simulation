...
xp =  tveq_logbarrier(x0, A_f, At_f, measured, lbtol, 10, slqtol, slqmaxiter); 

% for i=1:3
%     % This is not good. I'd like to have 'opts' here to control the W in ||Wx||_tv. 
%     Wi = diag(1 ./ abs(xp).^2); 
%     x0 = Wi * xp;
%     
%     xp =  tveq_logbarrier(x0, A_f, At_f, measured, lbtol, mu, slqtol, slqmaxiter); 
%     
% end

% Another option is to do a Continuation with the lbtol decreasing.
lbtol = 1e-2;
for i = 1:3
    xp =  tveq_logbarrier(x0, A_f, At_f, measured, lbtol, mu, slqtol, slqmaxiter); 
    x0 = xp;
...
Ip = reshape(xp, sz, sz)';
figure, image(Ip,'CDatamapping','scaled'),set(gca,'FontSize',20);
title('rec balanced Pos/Neg fix. Per 12.5%');
